...
*Prepared for investors and institutional partners. Every statement below is written from the current Northcue codebase and verified against the source on 3 July 2026. This document describes what the code does today. It is not marketing material, and it does not claim outcomes that the code cannot support. This edition adds visuals to the verified position; no claim has been changed or weakened.*

---

## 1. Executive summary

Northcue is a privacy by design service that turns a confusing official document into a few calm cue cards, and it is built so that the document itself does not need to be kept. Raw uploads are deleted immediately after processing on every path, and the extracted text lives only in server memory under a short time limit before it is cleared. Only anonymous, safe metadata is written to the database, never the contents of a document. To phrase the cue cards clearly, the text is sent once to an AI provider with structured identifiers redacted first and with provider side storage switched off, though a name or postal address written in ordinary prose can still be included, which the product discloses to the user. The posture is consistent with the UK GDPR principles of data minimisation and storage limitation, and the honest boundaries of that posture are stated plainly in the limitations section.

![Privacy position at a glance](assets_investor/callouts.png)

---

## 2. How your document flows, from upload to deletion

The diagram below traces a single document through the system. It shows visually that the uploaded file is deleted after processing, that the extracted text lives only briefly in memory and is then swept, and that the only thing which persists is anonymous metadata.

![The journey of a document from upload to deletion](assets_investor/dataflow.png)

The two rose markers show where the uploaded file is deleted and where the in memory text is swept. The single sage box shows the only thing that remains once a document has been turned into cue cards.

---

## 3. How Northcue compares

The table below sets Northcue beside a typical generic AI document summariser. It lists only capabilities where Northcue's position is verified in the code, and it describes the generic case with words such as typically and commonly rather than naming any product.

| Capability | Northcue | A typical AI document summariser |
| --- | --- | --- |
| Stores your uploaded document | No, deleted after processing | Commonly retained |
| Stores extracted text durably | No, held in memory only under a time limit | Commonly stored |
| Sends unredacted structured identifiers to the AI provider | No, emails, phone numbers, and other identifiers are redacted first | Commonly sends the raw text |
| Safety decided independently of the AI model | Yes, a deterministic rules layer decides safety on every path | Typically relies on the model |
| Refuses suspicious or unreadable uploads | Yes, suspected scams, non documents, and unreadable files are declined | Typically summarises anything |
| Requires an account to use | No, only an anonymous session identifier | Commonly requires an account |
...
## 5. Data handling with the AI provider
...
## 8. Honest limitations and pending items
...
## 9. Closing statement of posture
