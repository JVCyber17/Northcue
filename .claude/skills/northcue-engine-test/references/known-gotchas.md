...
**The AI pass only modifies `structured_result`, `display_text`, and `tts_script` — NEVER the engine's `output.cards`.**
At the engine level these are separate objects. BUT do not conclude "the AI pass is invisible to users" — the FRONTEND does not render `output.cards`. `normalizeApiResult` → `cardsFromStructuredResult` → `structuredCardToUiCard` (public/app.js) maps each `structured_result` card's `simple_explanation` → the headline and `key_points` → the bullets. So for good-quality documents (AI pass active) the headlines and bullets users actually see are the AI-rewritten `structured_result`. The rules `output.cards` are only the skip-AI fallback. (An earlier version of this note said users always see the rules version — that was wrong; verified live via the running server.)

**Headlines are tightened in the AI prompt; detail relocates into key_points.**
The system prompt instructs short, single-sentence headlines (~12 words), with supporting amounts/dates/specifics pushed into `key_points` (every card has them). Per-card cores: Card 1 = type + sender + key amount (bills); Card 4 = just the date, one sentence ("Your appointment is on 1 July 2026."); Card 5 consequence = attribution + hedge + short consequence in one sentence (prevents the mid-word truncation seen when a long consequence hit `max_output_tokens`). A prompt rule also forbids two cards sharing the same generic line (the adjacent-duplicate bug). The rules-engine `inferSummary` government Card-1 templates were tightened to match (dropped the trailing "with a date of {date}" clause — the date lives on Card 4).

**Shortening Card 3 can produce a pay instruction the strip filter missed — FIXED on both layers.**
When the "short core action" rule was added, gpt-4.1-mini turned the Barclays action headline into "Pay the £320.00 owed by 24 June 2026." — a direct pay instruction (prompt-level "do not tell the user to pay" is not reliably honoured). Worse, `stripAiViolations`'s pay-pattern caught "pay £320" but NOT "pay **the** £320" (the "the" slipped between "pay" and the amount). Fixed two ways: (1) the prompt now steers Card 3 to a SAFE action ("Check the amount and the due date" / "contact the sender with trusted details") and explicitly forbids "Pay £X" phrasing even on money documents; (2) the `_AI_PAY_PATTERNS` first regex now allows an optional `the\s+` before the amount/keyword (`pay\s+(?:the\s+)?(?:[£$€]\S+|\d+|amount|...)`). If a pay instruction ever reaches a headline again, check both the prompt steering and this pattern. Note: when the strip DOES fire on two cards it can replace both with the same generic line (an adjacent duplicate) — so the prompt steering (avoiding the instruction in the first place) is the primary defence, the strip is the backstop.

**The AI pass is hard-gated on low-quality input — it does not run on borderline or poor documents.**
A code gate in `applySafetyPassAndRecordAiStatus` (named `applyAiStructuredResult` until 2 August 2026) (added after prompt-based suppression was confirmed unreliable with gpt-4.1-mini) skips the AI pass entirely when `input_quality === "borderline"` or `"poor"`, or `garbled_by_ocr === true`. When the gate fires: `ai_used: false`, `ai_status: "skipped"`, `ai_error_code: "low_quality_input"`, `duration: 0ms`. No API call is made. The rules engine output is returned unchanged, preserving the correct uncertainty language in the `structured_result` / TTS layer. Verified live: garbled OCR bill (DOC_11 / DOC 3 in `_ai_compare.js`) shows `ai_used=NO (skipped / low_quality_input)` and `[no changes]`.
