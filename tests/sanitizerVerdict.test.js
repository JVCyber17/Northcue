...
// applySafetyPassAndRecordAiStatus then validated that, which is the engine's own result
...
const { applySafetyPassAndRecordAiStatus } = require(path.join(__dirname, "..", "src", "services", "aiStructuredResultService"));
const {
...
async function runWithCandidate(text, id, mutate) {
  ...
  try {
    const run = await applySafetyPassAndRecordAiStatus({ rulesRun: engineFor(text, id), extractedText: text, language: "en" });
    return { run, fallback };
  } finally {
