...
| 13 | check next step, base (~3241) | `trust.safe_next_step` or `banner.text`, or the served card 3 answer raw (`servedActionAnswer()`) when the decline leads the hero (24 September 2026) | none; the no-rush prefix interpolates AFTER lookup and never wraps the decline | exact; the served AI-path decline is already the bank line and passes through unmatched, by design |
...
| 16 | `isDeclineLine()` (added 24 September 2026, stop false reassurance; the essence card 3 stand-down, the full-view step filter, and the Document check hero and urgency all route through it) | `ESSENCE_DECLINE_LINE` (a module constant, the engine's decline sentence verbatim) | none; the raw constant goes to the lookup and the equality comparison happens on the translated result | exact `tpl.action.none_found`. Used as an equality probe against served raw text, so the AI-path decline (which the server enforces to be the bank line, byte for byte) is recognised in every language by every consumer |
