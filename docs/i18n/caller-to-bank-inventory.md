...
| 16 | essenceLineFor card 3 decline stand-down (added 24 September 2026, stop false reassurance) | `ESSENCE_DECLINE_LINE` (a module constant, the engine's decline sentence verbatim) | none; the raw constant goes to the lookup and the equality comparison happens on the translated result | exact `tpl.action.none_found`. Used as an equality probe against the served raw answer, so the AI-path decline (which the server enforces to be the bank line, byte for byte) stands the essence layer down in every language |
